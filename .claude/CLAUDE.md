...
- **Reglas:** automatizaciones del usuario construidas como un workflow de nodos, al estilo de n8n. Un disparador (media añadida, editada, restaurada o ejecución manual) lleva a condiciones (tags, nombre o autor —con uno o varios valores—, tamaño, resolución, orientación, tipo, favorito, si estuvo en la papelera) con salidas "True" y "False", y estas a acciones (añadir o quitar tags, marcar o desmarcar favorito, añadir a un álbum). Solo se ejecutan las reglas activas y completas; también se pueden aplicar a toda la biblioteca con "Run rule".
...
- **Resolución:** `media.width` y `media.height` guardan la resolución con la que se ve el original (ya girada según EXIF o la rotación del vídeo). Se leen al subir (`generateMediaDerivatives`) y al vincular desde Drive (`imageMediaMetadata`/`videoMediaMetadata`); si no se puede leer quedan a `NULL`. Para las medias anteriores: `npm run dimensions:backfill --prefix server`.
