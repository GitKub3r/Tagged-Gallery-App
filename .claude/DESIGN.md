...
- **Configuración de un nodo:** `MediaFormModal` compacto que se abre con un clic en el nodo (arrastrarlo no lo abre), con Enter o con el botón editar. Añadir un nodo no la abre: el nodo aparece seleccionado en el lienzo. El modal muestra la descripción del nodo, sus campos y el pie "Delete node" (`dangerGhost`, a la izquierda), "Cancel" y "Apply". Los cambios se guardan en la regla con "Save".
