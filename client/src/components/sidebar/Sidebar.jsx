                ...
                <nav className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3" aria-label="Main navigation">
                    ...
                    {user?.type === "dev" ? (
                        <section className="border-t border-neutral-200 pt-3 dark:border-neutral-800" aria-label="Developer navigation">
                            ...
                            <button type="button" className={`${navItemClassName(false, isCollapsed)} w-full! ${forceLoading ? "border-neutral-950! bg-neutral-950! text-white! dark:border-white! dark:bg-white! dark:text-neutral-950!" : "border-transparent! bg-transparent! text-neutral-600! hover:border-neutral-200! hover:bg-neutral-100! hover:text-neutral-950! dark:text-neutral-400! dark:hover:border-neutral-700! dark:hover:bg-neutral-800! dark:hover:text-white!"}`} onClick={() => setForceLoading(!forceLoading)} aria-pressed={forceLoading} aria-label="Toggle loading preview mode" title={isCollapsed ? "Loading mode" : undefined}>
                                <FontAwesomeIcon icon={faCode} className="w-5 shrink-0" aria-hidden="true" />
                                <SidebarLabel isCollapsed={isCollapsed}>Loading mode</SidebarLabel>
                                <span className={`ml-auto h-5 w-9 rounded-full p-0.5 transition-colors ${forceLoading ? "bg-white dark:bg-neutral-950" : "bg-neutral-300 dark:bg-neutral-700"} ${isCollapsed ? "xl:hidden" : ""}`} aria-hidden="true"><span className={`block h-4 w-4 rounded-full transition-transform ${forceLoading ? "translate-x-4 bg-neutral-950 dark:bg-white" : "bg-white dark:bg-neutral-300"}`} /></span>
                            </button>
                        </section>
                    ) : null}
